...
# 念のため結果CSVは毎回新規。出力: km2/all/results-frontreco.csv
# ============================================================================
set -u
NS=exp
...
CSV=${CSV:-$REPO/km2/all/results-frontreco.csv}
LOG=${LOG:-$REPO/km2/all/frontreco.log}
...
run_load() { # run_time capture
  ...
  local logfile="$REPO/km2/all/last-logs-frontreco.txt"
  printf '%s\n' "$logs" > "$logfile"
