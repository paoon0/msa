...
# 出力: km2/all/results-replicasweep.csv (列頭 replicas)
# ============================================================================
set -u
NS=exp
...
CSV=${CSV:-$REPO/km2/all/results-replicasweep.csv}
LOG=${LOG:-$REPO/km2/all/replicasweep.log}
...
run_load() { # run_time capture replicas
  ...
  local logfile="$REPO/km2/all/last-logs-replicasweep.txt"
  printf '%s\n' "$logs" > "$logfile"
