...
# 出力: km2/all/results-hpacompare.csv (arm, users, ..., scale_replicas, scale_detail)
# ============================================================================
set -u
NS=exp
...
CSV=${CSV:-$REPO/km2/all/results-hpacompare.csv}
LOG=${LOG:-$REPO/km2/all/hpacompare.log}
...
run_load() { # run_time capture arm scaleset...
  ...
  local logfile="$REPO/km2/all/last-logs-hpacmp.txt"
  printf '%s\n' "$logs" > "$logfile"
