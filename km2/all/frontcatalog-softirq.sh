...
# 念のため結果CSVは毎回新規。出力: km2/all/results-frontcatalog.csv
# ============================================================================
set -u
NS=exp
...
CSV=${CSV:-$REPO/km2/all/results-frontcatalog.csv}
LOG=${LOG:-$REPO/km2/all/frontcatalog.log}
...
run_load() { # run_time capture
  ...
  local logfile="$REPO/km2/all/last-logs-frontcatalog.txt"
  printf '%s\n' "$logs" > "$logfile"
