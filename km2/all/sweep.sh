...
CSV=${CSV:-$REPO/km2/all/results-sweep.csv}
LOG=${LOG:-$REPO/km2/all/sweep.log}
...
run_load() { # arm run_time capture(1=記録/0=捨て)
  ...
  local logfile="$REPO/km2/all/last-logs-sweep-$arm.txt"
  printf '%s\n' "$logs" > "$logfile"
