...
# normal / mega は測り直さず、既存のスイープ値(USERS=80: normal 1.031 / mega 0.459)と並べる。
...
# 公平のため Istio無しに揃える(normal/mega と同条件)。負荷も同じ USERS=80 RATE=40。
...
exec > >(tee -a "$LOG") 2>&1
echo "================ PAYMAIL START $(date -Is) ns=$NS users=$LG_USERS rate=$LG_RATE warmup=$WARMUP measure=$MEASURE ================"
echo "arm,reqcount,fails,p50_ms,p90_ms,p99_ms,avg_ms,rps,softirq_cores,base_softirq,system_cores,base_system,app_cores,base_app,node_cores,base_node,softirq_mc_per_req,system_mc_per_req,app_mc_per_req,node_mc_per_req" > "$CSV"
...
if b in data and e in data:
    ...
    if agg:
        ...
        rec=[arm,g("Request Count"),g("Failure Count"),g("50%"),g("90%"),g("99%"),
