...
# 念のため結果CSVは毎回新規。出力: km2/all/results-outpy.csv
# ============================================================================
set -u
NS=exp
...
CSV=${CSV:-$REPO/km2/all/results-outpy.csv}
LOG=${LOG:-$REPO/km2/all/outpy.log}
...
run_load() { # run_time capture
  ...
  local logfile="$REPO/km2/all/last-logs-outpy.txt"
  printf '%s\n' "$logs" > "$logfile"
