...
# 出力: km2/all/results-hpasweep.csv (列頭 users, 末尾 total_replicas)
# ============================================================================
set -u
NS=exp
...
CSV=${CSV:-$REPO/km2/all/results-hpasweep.csv}
LOG=${LOG:-$REPO/km2/all/hpasweep.log}
...
run_load() { # run_time capture
  ...
  local logfile="$REPO/km2/all/last-logs-hpasweep.txt"
  printf '%s\n' "$logs" > "$logfile"
