...
#
# 使い方:  bash km2/experiments/compare.sh
...
LOAD=$REPO/km2/experiments/loadgen-csv.yaml      # テンプレ(RUN_TIME はここから差し替える)
LOADGEN=/tmp/loadgen-gen.yaml            # RUN_TIME を差し込んだ実行用マニフェスト
CSV=${CSV:-$REPO/km2/experiments/results-cpu.csv}
LOG=${LOG:-$REPO/km2/experiments/compare.log}
...
run_load() { # cycle arm run_time capture(1=記録/0=捨て)
  ...
  local logfile="$REPO/km2/experiments/last-logs-$arm.txt"
  printf '%s\n' "$logs" > "$logfile"
