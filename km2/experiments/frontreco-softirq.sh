#!/usr/bin/env bash
# ============================================================================
# frontreco(frontend + recommendationservice を1Podに同居し localhost で到達) の softirq を測る(単独)。
# normal / mega は測り直さず、本走 results-cpu.csv(compare.sh, USERS=80: normal≈1.18 / mega≈0.56)と並べる。
# frontreco: reco を frontend の Pod に同居し RECOMMENDATION_SERVICE_ADDR=localhost:8090(reco は 8080→8090 remap)。
#   他ホップ(productcatalog/currency/cart/shipping/checkout/ad)は ClusterIP。reco→catalog も ClusterIP。
#   reco は frontend からしか呼ばれないので recommendationservice Service は無し。
#   【バイト vs パケット 判別】reco はパケット中(≒cart 2050pkts/s)だがバイト大(544KB/s)。
#     softirq 削減が frontcart 並み(-5%)なら"パケット説"、frontcatalog 寄り(-10%↑)なら"バイト説"。
...
# 念のため結果CSVは毎回新規。出力: km2/experiments/results-frontreco.csv
# ============================================================================
set -u
NS=exp
...
LOADGEN=/tmp/loadgen-frontreco.yaml
CSV=${CSV:-$REPO/km2/experiments/results-frontreco.csv}
LOG=${LOG:-$REPO/km2/experiments/frontreco.log}
...
# reco は frontend Pod に同居 → recommendationservice Deployment は無い。
FRONTRECO_DEPLOYS=(frontend checkoutservice cartservice redis-cart productcatalogservice \
        currencyservice paymentservice emailservice shippingservice adservice)
PROM="http://prometheus-grafana-kube-pr-prometheus.monitoring.svc:9090"

CYCLES=${CYCLES:-3}
exec > >(tee -a "$LOG") 2>&1
echo "================ FRONTRECO START $(date -Is) ns=$NS users=$LG_USERS rate=$LG_RATE warmup=$WARMUP measure=$MEASURE cycles=$CYCLES ================"
...
# 既存(normal/mega)の残骸を全消ししてから frontreco を張る
echo "---- 既存トポロジを全削除 ----"
...
echo "---- deploy FRONTRECO (frontend+recommendation 同居・localhost・Istio無し) ----"
for y in "$REPO"/km2/frontreco/*.yaml; do
  case "$y" in *kustomization.yaml|*loadgenerator.yaml) continue;; esac
  kubectl apply -f "$y" -n "$NS"
done
for d in "${FRONTRECO_DEPLOYS[@]}"; do
  kubectl patch deploy/"$d" -n "$NS" --type=merge -p '{"spec":{"template":{"metadata":{"annotations":{"sidecar.istio.io/inject":"false"}}}}}' || true
done
for d in "${FRONTRECO_DEPLOYS[@]}"; do kubectl rollout status deploy/"$d" -n "$NS" --timeout="$ROLLOUT_TIMEOUT" || true; done
...
run_load() { # run_time capture
  ...
  local logfile="$REPO/km2/experiments/last-logs-frontreco.txt"
  printf '%s\n' "$logs" > "$logfile"
  python3 - "${CYC:-1}" "frontreco" "$CSV" "$logfile" \
