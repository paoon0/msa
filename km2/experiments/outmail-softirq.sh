...
# 念のため結果CSVは毎回新規。出力: km2/experiments/results-outmail.csv
# ============================================================================
set -u
NS=exp
...
CSV=${CSV:-$REPO/km2/experiments/results-outmail.csv}
LOG=${LOG:-$REPO/km2/experiments/outmail.log}
...
run_load() { # run_time capture
  ...
  local logfile="$REPO/km2/experiments/last-logs-outmail.txt"
  printf '%s\n' "$logs" > "$logfile"
