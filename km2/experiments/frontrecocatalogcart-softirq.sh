#!/usr/bin/env bash
# ============================================================================
# frontrecocatalogcart(front4): frontend+recommendation+productcatalog+cart(+redis) を1Podに同居し
#   内部を全部 localhost 化した「閲覧パス束ね」の softirq/req を測る(用量反応の上側=megaに近づくか)。
# normal/mega は測り直さず本走(normal≈1.18/mega≈0.56)と並べる。条件は本走一致(3cyc/WARMUP3m/MEASURE5m/80/1)。
# HPA(CPU70%,1-5)は yaml 同梱。固定208rpsでは基本1レプリカのままのはず→各計測でレプリカ数とHPA状態をログし検知。
#   (=「HPAを付けても通常負荷ではスケールせず、softirq/reqはこの束ねの値」を示す第一歩)
# 出力: km2/experiments/results-front4.csv
# ============================================================================
set -u
NS=exp
...
LOADGEN=/tmp/loadgen-front4.yaml
CSV=${CSV:-$REPO/km2/experiments/results-front4.csv}
LOG=${LOG:-$REPO/km2/experiments/front4.log}
DIR=$REPO/km2/frontrecocatalogcart
...
# reco/catalog/cart/redis は frontend Pod に同居 → 別 Deployment 無し。
FRONT4_DEPLOYS=(frontend checkoutservice currencyservice paymentservice emailservice shippingservice adservice)
PROM="http://prometheus-grafana-kube-pr-prometheus.monitoring.svc:9090"

CYCLES=${CYCLES:-3}
exec > >(tee -a "$LOG") 2>&1
echo "================ FRONT4 START $(date -Is) ns=$NS users=$LG_USERS rate=$LG_RATE warmup=$WARMUP measure=$MEASURE cycles=$CYCLES ================"
...
for d in "${FRONT4_DEPLOYS[@]}"; do
  kubectl patch deploy/"$d" -n "$NS" --type=merge -p '{"spec":{"template":{"metadata":{"annotations":{"sidecar.istio.io/inject":"false"}}}}}' || true
done
for d in "${FRONT4_DEPLOYS[@]}"; do kubectl rollout status deploy/"$d" -n "$NS" --timeout="$ROLLOUT_TIMEOUT" || true; done
echo "  HPA状態: $(kubectl get hpa frontend -n "$NS" --no-headers 2>/dev/null)"
...
run_load() { # run_time capture
  ...
  if [ "$capture" = "1" ]; then
    ...
    hpa=$(kubectl get hpa frontend -n "$NS" --no-headers 2>/dev/null | awk '{print $3" reps="$6}')
    echo "  $(date -Is) CPU: softirq=${softirq_cores} app=${app_cores} node=${node_cores} | frontend_replicas=${reps} HPA(${hpa})"
  fi
  kubectl delete job loadgenerator -n "$NS" --ignore-not-found >/dev/null 2>&1
  [ "$capture" != "1" ] && { echo "  ウォームアップ完了(捨て)"; return; }
  local logfile="$REPO/km2/experiments/last-logs-front4.txt"
  printf '%s\n' "$logs" > "$logfile"
  python3 - "${CYC:-1}" "front4" "$CSV" "$logfile" \
