#!/usr/bin/env bash
# ============================================================================
# frontcatalog(frontend + productcatalogservice を1Podに同居し localhost で到達) の softirq を測る(単独)。
# normal / mega は測り直さず、本走 results-cpu.csv(compare.sh, USERS=80: normal≈1.18 / mega≈0.56)と並べる。
# frontcatalog: productcatalog を frontend の Pod に同居し、frontend は
#   PRODUCT_CATALOG_SERVICE_ADDR=localhost:3550 で到達(=最重エッジ frontend↔productcatalog を loopback化)。
#   他ホップ(currency/cart/reco/shipping/checkout/ad)は ClusterIP。productcatalogservice Service は
#   app=frontend Pod に向き、recommendation/checkout は従来どおり ClusterIP で到達。
#   【用量反応の"効く点"】軽ホップ(outmail/outpy)と違い、重エッジなので softirq 明確低下を予想(0.8-0.9台)。
...
# 念のため結果CSVは毎回新規。出力: km2/experiments/results-frontcatalog.csv
# ============================================================================
set -u
NS=exp
...
LOADGEN=/tmp/loadgen-frontcatalog.yaml
CSV=${CSV:-$REPO/km2/experiments/results-frontcatalog.csv}
LOG=${LOG:-$REPO/km2/experiments/frontcatalog.log}
...
# productcatalog は frontend Pod に同居 → productcatalogservice という Deployment は無い。
FRONTCATALOG_DEPLOYS=(frontend checkoutservice cartservice redis-cart currencyservice \
        paymentservice emailservice shippingservice recommendationservice adservice)
PROM="http://prometheus-grafana-kube-pr-prometheus.monitoring.svc:9090"

CYCLES=${CYCLES:-3}
exec > >(tee -a "$LOG") 2>&1
echo "================ FRONTCATALOG START $(date -Is) ns=$NS users=$LG_USERS rate=$LG_RATE warmup=$WARMUP measure=$MEASURE cycles=$CYCLES ================"
...
# 既存(normal/mega)の残骸を全消ししてから frontcatalog を張る
echo "---- 既存トポロジを全削除 ----"
...
echo "---- deploy FRONTCATALOG (frontend+productcatalog 同居・localhost・Istio無し) ----"
for y in "$REPO"/km2/frontcatalog/*.yaml; do
  case "$y" in *kustomization.yaml|*loadgenerator.yaml) continue;; esac
  kubectl apply -f "$y" -n "$NS"
done
for d in "${FRONTCATALOG_DEPLOYS[@]}"; do
  kubectl patch deploy/"$d" -n "$NS" --type=merge -p '{"spec":{"template":{"metadata":{"annotations":{"sidecar.istio.io/inject":"false"}}}}}' || true
done
for d in "${FRONTCATALOG_DEPLOYS[@]}"; do kubectl rollout status deploy/"$d" -n "$NS" --timeout="$ROLLOUT_TIMEOUT" || true; done
...
run_load() { # run_time capture
  ...
  local logfile="$REPO/km2/experiments/last-logs-frontcatalog.txt"
  printf '%s\n' "$logs" > "$logfile"
  python3 - "${CYC:-1}" "frontcatalog" "$CSV" "$logfile" \
...
kubectl delete pod promq -n "$NS" --ignore-not-found >/dev/null 2>&1
echo "================ FRONTCATALOG DONE $(date -Is) ================"
