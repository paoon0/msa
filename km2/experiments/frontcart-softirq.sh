#!/usr/bin/env bash
# ============================================================================
# frontcart(frontend + cartservice + redis を1Podに同居し localhost で到達) の softirq を測る(単独)。
# normal / mega は測り直さず、本走 results-cpu.csv(compare.sh, USERS=80: normal≈1.18 / mega≈0.56)と並べる。
# frontcart: cart と redis を frontend の Pod に同居し、2ホップを loopback 化:
#   ① frontend→cart : CART_SERVICE_ADDR=localhost:7070
#   ② cart→redis    : REDIS_ADDR=localhost:6379
#   他ホップ(productcatalog/currency/reco/shipping/checkout/ad)は ClusterIP。
#   cartservice Service は app=frontend Pod に向き、checkout は ClusterIP で従来どおり到達。
#   redis-cart Service は無し(redis は同居先 cart のみが localhost で使う)。
#   【用量反応の中間点】cart(~2147pkts/s)+redis(~834)を loopback → frontcatalog(7182,-16%)より軽く ad(270)より重い。
...
# 念のため結果CSVは毎回新規。出力: km2/experiments/results-frontcart.csv
# ============================================================================
set -u
NS=exp
...
LOADGEN=/tmp/loadgen-frontcart.yaml
CSV=${CSV:-$REPO/km2/experiments/results-frontcart.csv}
LOG=${LOG:-$REPO/km2/experiments/frontcart.log}
...
# cart と redis は frontend Pod に同居 → cartservice / redis-cart Deployment は無い。
FRONTCART_DEPLOYS=(frontend checkoutservice productcatalogservice currencyservice \
...
CYCLES=${CYCLES:-3}
exec > >(tee -a "$LOG") 2>&1
echo "================ FRONTCART START $(date -Is) ns=$NS users=$LG_USERS rate=$LG_RATE warmup=$WARMUP measure=$MEASURE cycles=$CYCLES ================"
...
# 既存(normal/mega)の残骸を全消ししてから frontcart を張る
echo "---- 既存トポロジを全削除 ----"
...
echo "---- deploy FRONTCART (frontend+cart+redis 同居・localhost・Istio無し) ----"
for y in "$REPO"/km2/frontcart/*.yaml; do
  case "$y" in *kustomization.yaml|*loadgenerator.yaml) continue;; esac
  kubectl apply -f "$y" -n "$NS"
done
for d in "${FRONTCART_DEPLOYS[@]}"; do
  kubectl patch deploy/"$d" -n "$NS" --type=merge -p '{"spec":{"template":{"metadata":{"annotations":{"sidecar.istio.io/inject":"false"}}}}}' || true
done
for d in "${FRONTCART_DEPLOYS[@]}"; do kubectl rollout status deploy/"$d" -n "$NS" --timeout="$ROLLOUT_TIMEOUT" || true; done
...
run_load() { # run_time capture
  ...
  local logfile="$REPO/km2/experiments/last-logs-frontcart.txt"
  printf '%s\n' "$logs" > "$logfile"
  python3 - "${CYC:-1}" "frontcart" "$CSV" "$logfile" \
