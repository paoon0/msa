...
metadata: 
  ...
  modified: 2026-08-18T13:14:27.442Z
---

**★★★2026-08-29 目的関数の再定式化(ユーザ指摘2件で訂正。ここが最新)。**
ユーザ指摘①「最適解は粒度損とパフォーマンスの両方から言うべきでは」/②「境界点を見つける話はどうなった」。両方とも正しく、こちらの整理が狭くなっていた。

- **目的関数を書き直す: 効率 = SLOを満たせる最大負荷 ÷ そのとき予約したコア [周/s per core]。**
  softirq削減は見出し指標ではなく分子を伸ばすメカニズムの一つ(A4)にすぎない(元々 [[bundling-merit-question]] にそう書いてある)。粒度損は分母を膨らませる一項。**得(softirq 0.05-0.2コア)だけを損(1-2コア)と比べると必ず「束ねるな」になる=比較の仕方が誤り。**
- **★実データで粒度損の順位と性能の順位が完全に逆転(results-loadsweep.csv 再解析)。**
  | 束ね | 粒度損@200(関係のみ) | SLO満たす最大負荷 | 効率 |
  |---|---|---|---|
  | normal | 基準 | 200周/s | 20.7 |
  | front+catalog | **+0.00(最良)** | **200周/s(最低)** | 21.2 |
  | front+reco | **+1.82(最悪)** | **300周/s(最高)** | **22.5** |
  | front+cart | +1.45 | 250周/s | 20.3 |
  ⇒ **「粒度損の小さい組を選べ」は誤った答えを出す。** (但し250以上はスロットリング崩壊に汚染。確定には実験B必須=Bの理由が「測定の邪魔の除去」から「縦軸そのものを測る前提」に格上げ)
- **★境界点は消えていない。横軸が「負荷の大きさ」→「需要のズレ」に移った。**
  旧(2026-07-07): 高負荷で重い親が多レプリカ化し軽い相方が無駄増殖=束ねが損に転じる、境目は重さ比。
  訂正理由(a) 枠を適正化すると全サービスが1台あたりk周/sで70%到達=必要台数一致=**負荷軸上に境界が存在しない**([[rightsizing-experiment]])。(b) 実測が旧予測を否定=**高負荷ほど束ねが強い**(300周/sで分離は崩壊、frontrecoはp50 23msで完走、予約コアも束ねが少ない)。
  ⇒ **最終主張の図: 縦軸=効率(SLO容量÷予約コア)、横軸=需要のズレ(閲覧型:購入型の比率)。束ね曲線と分離曲線の交点=境界点。** 「どの組を束ねるべきか=需要の高さが揃っている組」。ICTerは通信量だけで選ぶのでズレた組を選んでしまう=反論の形。
- ⇒ 順番: ①実験B(縦軸の前提) ②効率カーブ測定(縦軸の確立) ③実験2=混合ワークロード(横軸を作り境界点を測る=主張の完成)。**実験2は追加実験ではなく本体。**

**★★2026-08-18 ユーザ報告=指導教授から「束ねる組み合わせは“定性的に”見つけても構わない」と了承済み。** ⇒ ゴールは「損の係数まで当てる定量的予測式の較正」ではなく **定性的な選別則(ふるい)+実験による順位の裏取り**でよい。具体的には ①通信バイトが重いエッジか ②利用率カーブの高さ(=一緒に70%へ届くか)が揃う群か ③構造上の専属(in-degree=1、ただし状態持ちredisは除外)か、の3つのふるいで候補を絞り、**ルールが選ぶ組 vs 選ばない組を実際に束ねて優劣が予測通りか**を示せば到達点。⇒ 利用率カーブ測定は「群分け(熱い/冷たい)が見える精度」で足り、レート点数・反復数を絞ってよい(較正用の高精度は不要)。混合負荷(時間帯ズレ)も定性で可。
...
## ★★2026-09-07 更新(実験9/4の解析 `km2/experiments/summer2026/analysis-scale3.md` より)
1. **便益の加算は「仲間どうしのエッジ」も足す**。frontend+reco+catalog を束ねると
   frontend↔catalog・frontend↔reco に加えて **reco→catalog** も localhost 化される
   (`src/recommendationservice/recommendation_server.py:70`)。実測 softirq はペアの和 −20.3% に対し
   3つ束ね **−26.2%** で超加算。⇒ 目的関数の便益項 = **束ねた集合の中に取り込まれた全エッジ**の通信量の和。
2. **需要相関の測り方を訂正**。「CPU利用率の高さ(%)が揃うか」は**判定に使えない**。
   HPAが70%に張り付くよう台数を動かすので、どのサービスも54〜73%に見えてしまう(実測)。
   正しい判定量は **需要 = 利用率 × 台数**(枠を単位とした量)で、
   **需要 ÷ 枠 が同じ整数台数に落ちる相手と束ねる**。枠はその整数を揃えるためのつまみ。
3. **損も得も整数の閾値効果**。HPAの許容幅10%のため「増える=77%超/減る=63%未満」で、
   3台と4台の**両方が安定**な帯が存在する。同居によるCPU −2〜3%(連続量)が
   この分岐を跨がせると「1台分=枠まるごと」(離散量)の得になる。得の大きさは削減CPUに比例しない。
   ⇒ 予約枠は主指標にできない(条件付きの副指標)。主指標は CPU時間(cgroup由来)と通信コスト。
